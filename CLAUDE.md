...
- `components/shared/editors/{ClassEditor,TeacherEditor,RoomEditor}.tsx` → the **only** create/edit/bulk-timeoff modals. Each is presentational + `React.memo`: owns form state, calls `onSave(data)`; the parent picks the service call. Consumed by both the CRUD page AND the timetable view (`ClassesPage`/`TeachersPage`/`RoomsPage` and `timetable-view-api/EntityEditorProvider.tsx`) so the modal is **1:1 identical** everywhere. ⚠️ Do NOT re-inline an editor into a page or fork a timetable-only variant — that reopens the drift these were extracted to kill. Inline styles (Plus Jakarta Sans / Manrope, hex) are intentional here; match them, don't "Tailwindify."
- Timetable entity edit: grid headers (`timetable-view/grids.tsx`) call `useEntityEdit().openEditor(kind, id)` (stable, from `timetable-view/EntityEditContext.tsx`). `EntityEditorProvider` holds the `editing` state + renders the shared editor → opening a modal re-renders ONLY the provider, never the orchestrator/MainGrid/grids (state-colocation; `children` element identity is preserved). Keep editing state in the provider; never hoist it into `TimetableViewPageWithAPI`.
